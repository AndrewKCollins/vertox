...
VERTOX detects the build path from the project root:
...
VERTOX checks only the tools needed for the detected project type. A native SBF project does not require Anchor.

## Anchor version switching

If an Anchor project declares an Anchor version in `Anchor.toml`, VERTOX can switch the local Anchor CLI before building:
...
After a successful build, VERTOX copies every `.so` file from the project's `target/deploy/` directory into `--out-dir`.
...
You can then pass the copied `.so` file directly to [`vertox reverse`](reverse.md).

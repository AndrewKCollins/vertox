...
The `fetch` command retrieves any Solana account through RPC. Executable programs are saved as `fetched_program.so`; non-executable accounts are saved as `fetched_account.bin`.
...
* `--program-id`: The Solana program or account address to fetch.
* `--out-dir`: Directory where the fetched account data will be saved.
...
* Validates the address and confirms the account exists on-chain.
* Resolves upgradeable ProgramData accounts when the target is executable.
* Writes a clean ELF for programs or raw bytes for data accounts.
* Reports the output file path.
...
| Flag `executable` | Typical content                                                                                             | File saved by **fetch** |
| ----------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------- |
...
`fetch` detects this flag automatically:
...
Those 8 bytes uniquely identify the struct on-chain. `fetch` already prints them for any data account it downloads.

...
2. **Instruction Analysis**
   ...
   Using the `Analysis` struct from `anza-xyz/sbpf`, the tool walks through all valid instruction addresses, building:
...
- [`get_string_repr`](cfg.md#strings-from-rodata): Converts slices from `.rodata` into readable strings
- [`export_cfg_to_dot`](cfg.md#overview): Emits Graphviz-compatible control flow graphs
- [`disassemble_wrapper`](immediates.md#behind-the-scenes): Main entrypoint for disassembly + data extraction

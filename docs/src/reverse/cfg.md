...
VERTOX can extract a static control flow graph (CFG) from a compiled Solana sBPF program.  
...
VERTOX uses the static `Analysis` engine to:
...
Here’s a raw `.dot` snippet generated by VERTOX:
...
Here’s what a real VERTOX-generated CFG can look like:
